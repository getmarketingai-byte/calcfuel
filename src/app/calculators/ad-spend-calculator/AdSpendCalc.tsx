...
export default function AdSpendCalc() {
  ...
  const calculate = () => {
    ...
    setResult({ clicks, leads, revenue, roi: ((revenue - b) / b) * 100, roas: revenue / b });
  };
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 md:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          ...
          <input type="number" min="0.01" step="0.01" value={cpc} onChange={e => setCpc(e.target.value)} placeholder="e.g. 2.50"
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-orange-400 outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Conversion Rate (%)</label>
          <input type="number" min="0.01" max="100" step="0.1" value={convRate} onChange={e => setConvRate(e.target.value)} placeholder="e.g. 3.5"
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-orange-400 outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Average Deal / Order Value (AUD)</label>
          <input type="number" min="1" value={dealValue} onChange={e => setDealValue(e.target.value)} placeholder="e.g. 500"
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-orange-400 outline-none" />
        </div>
      </div>
      {error && <p className="text-red-500 text-sm mb-4">{error}</p>}
      <button onClick={calculate} className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded-xl transition-colors">Calculate Ad Projections</button>
      {result !== null && (
