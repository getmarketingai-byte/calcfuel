...
/**
 * Approximate consumption in litres of fuel per kilowatt-hour of electrical output,
 * at a working load. Small gensets are least efficient at light load, which is why the
 * load percentage matters more than the nameplate rating.
 *
...
/** Litres per US gallon, for the imperial display path. */
...
export default function GeneratorFuelCalc() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      // Rates are litres per kWh, so hourly burn is rate x output kW.
      let ratePerHour = FUEL_RATES[fuelType] * kw * loadPct;
      if (unit === "imperial" && fuelType !== "natural_gas") {
        ratePerHour = ratePerHour / L_PER_GAL; // litres to US gallons
      }
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex rounded-lg overflow-hidden border border-gray-200 dark:border-gray-600 text-sm">
          <button onClick={() => setUnit("imperial")} className={"px-3 py-1.5 font-medium transition-colors " + (unit === "imperial" ? "bg-orange-700 text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700")}>Imperial</button>
          <button onClick={() => setUnit("metric")} className={"px-3 py-1.5 font-medium transition-colors " + (unit === "metric" ? "bg-orange-700 text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700")}>Metric</button>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="generator-size-watts">Generator Size (watts)</label>
          <input id="generator-size-watts" type="number" inputMode="numeric" min="0" value={watts} onChange={e => setWatts(e.target.value)}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="load-percentage">Load Percentage (%)</label>
          <input id="load-percentage" type="number" inputMode="numeric" min="1" max="100" value={load} onChange={e => setLoad(e.target.value)}
            placeholder="e.g. 50"
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-orange-400 outline-none" />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Fuel Available ({fuelUnitLabel}) <span className="text-gray-700 dark:text-gray-300 text-xs">(optional)</span>
          </label>
          <input aria-label="Fuel Available () (optional" type="number" inputMode="decimal" min="0" value={fuelAvailable} onChange={e => setFuelAvailable(e.target.value)}
            placeholder={unit === "imperial" ? "e.g. 10" : "e.g. 38"}
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-orange-400 outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Outage Duration (days) <span className="text-gray-700 dark:text-gray-300 text-xs">(optional)</span>
          </label>
          <input aria-label="Outage Duration (days) (optional" type="number" inputMode="decimal" min="0" step="0.5" value={daysNeeded} onChange={e => setDaysNeeded(e.target.value)}
            placeholder="e.g. 3"
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-orange-400 outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Fuel Price (per {fuelUnitShort}) <span className="text-gray-700 dark:text-gray-300 text-xs">(optional)</span>
          </label>
          <input aria-label="Fuel Price (per ) (optional" type="number" inputMode="decimal" min="0" step="0.01" value={fuelPrice} onChange={e => setFuelPrice(e.target.value)}
      ...
      {result !== null && (
        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4" aria-live="polite">
          <div className="p-4 rounded-xl border bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
            <p className="text-xs text-gray-700 dark:text-gray-300 mb-1">Consumption Rate</p>
            <p className="text-2xl font-bold text-blue-700 dark:text-blue-300">{result.consumptionPerHour.toFixed(3)} {fuelUnitShort}/hr</p>
          </div>
          {result.runtimeHours !== null && (
            <div className="p-4 rounded-xl border bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
              <p className="text-xs text-gray-700 dark:text-gray-300 mb-1">Runtime on {fuelAvailable} {fuelUnitLabel}</p>
          ...
          {result.fuelForDays !== null && (
            <div className="p-4 rounded-xl border bg-orange-50 dark:bg-orange-950 border-orange-200 dark:border-orange-800">
              <p className="text-xs text-gray-700 dark:text-gray-300 mb-1">Fuel for {daysNeeded}-day Outage</p>
          ...
          {result.costForDays !== null && (
            <div className="p-4 rounded-xl border bg-purple-50 dark:bg-purple-950 border-purple-200 dark:border-purple-800">
              <p className="text-xs text-gray-700 dark:text-gray-300 mb-1">Fuel Cost ({daysNeeded} days)</p>
              <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">${result.costForDays.toFixed(2)}</p>
            </div>
          )}
        </div>
      )}
