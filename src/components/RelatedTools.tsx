...
export default function RelatedTools({ tools }: { tools: RelatedTool[] }) {
  return (
    <section data-boilerplate className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {tools.map((tool) => (
          <Link key={tool.slug} href={`/calculators/${tool.slug}`}
            ...
            <p className="text-xs text-gray-700 dark:text-gray-300 mt-1">{tool.description}</p>
          </Link>
        ))}
      </div>
    </section>
